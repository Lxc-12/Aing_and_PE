...
#        |             GDF15 and PE              |
#        +---------------------------------------+
## import the GDF 15(Growth differentiation factor 15) GWAS data into R.
library(vroom)
GDF15_gwas <- vroom('F:\\Aging_gwas\\GDF 15\\harmonised.qc.tsv',
                   col_names = TRUE)

colnames(GDF15_gwas)
GDF15_gwas$phenotype <- "GDF15"

library(TwoSampleMR)
library(MRInstruments)

## extract SNPs with p£¼5e-08.
GDF15_p <- subset(GDF15_gwas, p_value<5e-08)

## format the data.
GDF15_format <- format_data(GDF15_p, type = "exposure",
                             snps = NULL,
                             header = TRUE,
                             ...
                             log_pval = FALSE)
## clump the data
exposure_data <- clump_data(GDF15_format, clump_kb = 10000,clump_r2 = 0.001)
