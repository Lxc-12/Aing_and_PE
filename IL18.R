#        +---------------------------------------+
#        |         analysis IL18 and PE          |
#        +---------------------------------------+
## import the IL 18(interleukin-18) GWAS data into R.
library(vroom)
IL18_gwas <- vroom('F:\\Aging_gwas\\IL 18\\harmonised.qc.tsv',
                  col_names = TRUE)

colnames(IL18_gwas)
IL18_gwas$phenotype <- "IL18"

library(TwoSampleMR)
library(MRInstruments)

## extract SNPs with p£¼5e-08.
IL18_p <- subset(IL18_gwas, p_value < 5e-08)

## format the data.
IL18_format <- format_data(IL18_p, type = "exposure",
                            snps = NULL,
                            header = TRUE,
                            ...
                            log_pval = FALSE)
## clump the data
exposure_data <- clump_data(IL18_format, clump_kb = 10000, clump_r2 = 0.001)
