...
#        |         analysis  IL6 and PE          |
#        +---------------------------------------+
## import the IL 6(interleukin-6) GWAS data into R.
library(vroom)
IL6_gwas <- vroom('F:\\Aging_gwas\\IL 6\\harmonised.qc.tsv',
                            col_names = TRUE)

colnames(IL6_gwas)
IL6_gwas$phenotype <- "IL6"

library(TwoSampleMR)
library(MRInstruments)

## extract SNPs with p£¼5e-08.
IL6_p <- subset(IL6_gwas, p_value < 5e-08)

## format the data.
IL6_format <- format_data(IL6_p, type = "exposure",
                                      snps = NULL,
                                      header = TRUE,
                                      ...
                                      log_pval = FALSE)
## clump the data
exposure_data <- clump_data(IL6_format, clump_kb = 10000, clump_r2 = 0.001)
