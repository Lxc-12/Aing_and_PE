#        +---------------------------------------+
#        |       Frailty index and PE            |
#        +---------------------------------------+
## import Frailty Index GWAS data into R.
library(vroom)
FI_gwas <- vroom('F:\\Aging_gwas\\Frailty index\\GCST90020053_buildGRCh37.tsv',
                   col_names = TRUE)

colnames(FI_gwas)
FI_gwas$phenotype <- "Frailty index"

library(TwoSampleMR)
library(MRInstruments)

## extract SNPs with p£¼5e-08.
FI_p <- subset(FI_gwas, p_value<5e-08)

## format the data.
FI_format <- format_data(FI_p, type = "exposure",
                             snps = NULL,
                             header = TRUE,
                             ...
                             log_pval = FALSE)
## clump the data.
exposure_data <- clump_data(FI_format, clump_kb = 10000, clump_r2 = 0.001)
