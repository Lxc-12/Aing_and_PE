#               +---------------------------+
#               |   analysis CHIP and PE    | 
#               +---------------------------+
## import the CHIP(clonal hematopoiesis) GWAS data into R.
library(vroom)
CHIP_gwas <- vroom('F:\\Aging_gwas\\CHIP\\chOVERALL_0.6info_0.002MAF_filtered.txt',
                    col_names = TRUE)

colnames(CHIP_gwas)
CHIP_gwas$phenotype <- "clonal hematopoiesis"

library(TwoSampleMR)
library(MRInstruments)

## extract SNPs with p£¼5e-08.
CHIP_p <- subset(CHIP_gwas, P_BOLT_LMM<5e-08)

## format the data.
CHIP_format <- format_data(CHIP_p, type = "exposure",
                                      snps = NULL,
                                      header = TRUE,
                                      ...
                                      log_pval = FALSE)
## clump the data
exposure_data <- clump_data(CHIP_format, clump_kb = 10000, clump_r2 = 0.001)
